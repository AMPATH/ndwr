DELIMITER $$
CREATE PROCEDURE `build_NDWR_patient_art_extract`(
...
set 
  @primary_table := "ndwr_patient_art_extract";
set 
  @total_rows_written = 0;
set 
  @start = now();
set 
  @table_version = "ndwr_patient_art_extract_v1.1";
set 
...
set 
  @write_table = concat(
    "ndwr_patient_art_extract_temp_", 
    queue_number
  );
set 
  @queue_table = concat(
    "ndwr_patient_art_extract_build_queue_", 
    queue_number
  );
SET 
  @dyn_sql = CONCAT(
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' (select * from ndwr_patient_art_extract_build_queue limit ', 
    queue_size, ');'
  );
PREPARE s1 
from 
...
SET 
  @dyn_sql = CONCAT(
    'delete t1 from ndwr_patient_art_extract_build_queue t1 join ', 
...
set 
  @queue_table = "ndwr_patient_art_extract_sync_queue";
CREATE TABLE IF NOT EXISTS ndwr_patient_art_extract_sync_queue (person_id INT PRIMARY KEY);
set 
  @last_update = null;
SELECT 
...
set 
  @loop_start_time = now();
drop 
  temporary table if exists ndwr_patient_art_extract_build_queue__0;
SET 
  @dyn_sql = CONCAT(
    'create temporary table if not exists ndwr_patient_art_extract_build_queue__0 (person_id int primary key) (select * from ', 
...
DEALLOCATE PREPARE s1;
drop 
  temporary table if exists ndwr_patient_art_extract_interim;
SELECT 
  CONCAT('Creating interim table .. ');
create temporary table ndwr_patient_art_extract_interim (
  SELECT 
    distinct t1.PatientPK, 
    t1.SiteCode, 
    t1.PatientID, 
    t1.FacilityID, 
    t1.Emr, 
    ...
    if(
      ...
      DATEDIFF(t1.RegistrationDate, DOB)/ 365.25, 
      DATEDIFF(
        t1.arv_first_regimen_start_date, 
        t1.DOB
      )/ 365.25
    ...
    t1.gender as Gender, 
    case when RegistrationDate <= if(
      DATE(
        t1.arv_first_regimen_start_date
      ) = "1900-01-01", 
      "1997-01-01", 
      t1.arv_first_regimen_start_date
    ) then RegistrationDate when RegistrationDate > if(
      DATE(
        t1.arv_first_regimen_start_date
      ) = "1900-01-01", 
      "1997-01-01", 
      t1.arv_first_regimen_start_date
    ) then DATE_ADD(RegistrationDate, INTERVAL 30 DAY) end as StartARTDate, 
    case when RegistrationDate <= if(
      DATE(
        t1.arv_first_regimen_start_date
      ) = "1900-01-01", 
      ...
      "1997-01-01", 
      t1.arv_first_regimen_start_date
    ) then DATE_ADD(RegistrationDate, INTERVAL 30 DAY) end as PreviousARTStartDate, 
    ...
    CASE WHEN t1.cur_arv_line = 1 
    AND (
      DATEDIFF(t1.lastVisit, t1.DOB) / 365.25
    ) >= 25 THEN "Adult FirstLine" WHEN t1.cur_arv_line = 1 
    AND (
      DATEDIFF(t1.lastVisit, t1.DOB) / 365.25
    ) < 25 THEN "Child FirstLine" WHEN t1.cur_arv_line = 2 
    AND (
      DATEDIFF(t1.lastVisit, t1.DOB) / 365.25
    ) >= 25 THEN "Adult SecondLine" WHEN t1.cur_arv_line = 2 
    AND (
      DATEDIFF(t1.lastVisit, t1.DOB) / 365.25
    ) < 25 THEN "Child SecondLine" WHEN t1.cur_arv_line = 3 
    AND (
      DATEDIFF(t1.lastVisit, t1.DOB) / 365.25
    ) >= 25 THEN "Adult ThirdLine" WHEN t1.cur_arv_line = 3 
    AND (
      DATEDIFF(t1.lastVisit, t1.DOB) / 365.25
    ) < 25 THEN "Child ThirdLine" ELSE NULL END AS StartRegimenLineCategory, 
    ...
    null as DateCreated 
  FROM 
    ndwr.ndwr_all_patients_extract t1 
...
SET 
  @dyn_sql = CONCAT(
    'replace into ', @write_table, '(select * from ndwr_patient_art_extract_interim)'
  );
PREPARE s1 
from 
  @dyn_sql;
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' t1 join ndwr_patient_art_extract_build_queue__0 t2 using (person_id);'
  );
PREPARE s1 
from 
  @dyn_sql;
