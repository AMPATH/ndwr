DELIMITER $$
CREATE PROCEDURE `ndwr`.`build_NDWR_all_patients_extract`(
  IN query_type varchar(50), 
  IN queue_number int, 
  IN queue_size int, 
  IN cycle_size int, 
  IN log BOOLEAN
) BEGIN CREATE TABLE IF NOT EXISTS ndwr_all_patients_extract (
  `Pkv` VARCHAR(50) NULL, 
...
set 
  @primary_table := "ndwr_all_patients_extract";
set 
  @total_rows_written = 0;
set 
  @start = now();
set 
  @table_version = "ndwr_all_patients_v1.1";
set 
...
set 
  @write_table = concat(
    "ndwr_all_patients_temp_", queue_number
  );
set 
  @queue_table = concat(
    "ndwr_all_patients_build_queue_", 
    queue_number
  );
SET 
  @dyn_sql = CONCAT(
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' (select * from ndwr_all_patients_build_queue limit ', 
    queue_size, ');'
  );
PREPARE s1 
from 
...
SET 
  @dyn_sql = CONCAT(
    'delete t1 from ndwr_all_patients_build_queue t1 join ', 
...
DEALLOCATE PREPARE s1;
set 
  @queue_table = "ndwr_all_patients_sync_queue";
...
set 
  @loop_start_time = now();
drop 
  table if exists ndwr_all_patients_build_queue__0;
SET 
  @dyn_sql = CONCAT(
    'create temporary table if not exists ndwr_all_patients_build_queue__0 (person_id int primary key) (select * from ', 
...
DEALLOCATE PREPARE s1;
select 
  CONCAT('Creating soundex mapping ...');
drop 
  temporary table if exists ndwr_patient_pkv_occcupation_mapping;
create temporary TABLE ndwr_patient_pkv_occcupation_mapping(
  SELECT 
    q.person_id, 
    upi.identifier as 'NUPI', 
    p.gender, 
    p.birthdate, 
    ...
    CONCAT(
      p.gender, 
      n.given_name, 
      SOUNDEX(n.given_name), 
      SOUNDEX(n.family_name), 
      DATE_FORMAT(p.birthdate, '%Y')
    ) as 'Pkv' 
  FROM 
    ndwr_all_patients_build_queue__0 q 
    left join amrs.person p on (
      p.person_id = q.person_id 
      AND p.voided = 0
    ) 
    left join amrs.person_name n on (
      n.person_id = q.person_id 
      AND n.voided = 0
    ) 
    left join amrs.person_attribute a on (
      a.person_id = q.person_id 
      AND a.person_attribute_type_id = 42 
      AND a.voided = 0
    ) 
    left join amrs.concept_name cn on (
      cn.concept_id = a.value 
      and cn.locale_preferred = 1 
      AND a.value != 5622
    ) 
    left join amrs.patient_identifier i on (
      i.patient_id = q.person_id 
      AND i.identifier_type = 28 
      AND i.voided = 0
    ) 
    left join amrs.patient_identifier upi on (
      upi.patient_id = q.person_id 
...
select 
  CONCAT(
    'Creating ndwr_all_patients_interim table ...'
  );
drop 
  temporary table if exists ndwr_all_patients_interim;
CREATE temporary TABLE ndwr_all_patients_interim (
  SELECT 
    pm.Pkv as 'PKV', 
    pm.NUPI as 'NUPI', 
    t1.person_id AS 'PatientPK', 
    case when t1.visit_type in (
      23, 24, 119, 124, 129, 43, 80, 118, 120, 
      123
    ) THEN mfl2.mfl_code else mfl.mfl_code end as 'SiteCode', 
    REPLACE(pm.PatientID, "-", "") AS 'PatientID', 
    case when t1.visit_type in (
      23, 24, 119, 124, 129, 43, 80, 118, 120, 
      123
    ) THEN mfl2.mfl_code else mfl.mfl_code end as 'FacilityID', 
    'AMRS' AS Emr, 
    'Ampath Plus' AS 'Project', 
    mfl.Facility AS FacilityName, 
    pm.gender AS Gender, 
    pm.birthdate AS DOB, 
    CASE WHEN DATE(t1.enrollment_date) = '1900-01-01' 
    AND DATE(birthdate) <= '1997-01-01' THEN '1997-01-01' WHEN DATE(t1.enrollment_date) = '1900-01-01' 
    AND DATE(birthdate) > '1997-01-01' THEN DATE_ADD(birthdate, INTERVAL 30 DAY) WHEN DATE(t1.enrollment_date) > '1900-01-01' 
    AND DATE(birthdate) > DATE(t1.enrollment_date) THEN DATE_ADD(birthdate, INTERVAL 30 DAY) WHEN DATE(t1.enrollment_date) > '1900-01-01' 
    AND DATE(birthdate) <= DATE(t1.enrollment_date) THEN t1.enrollment_date END AS 'RegistrationDate', 
    CASE WHEN DATE(t1.enrollment_date) = '1900-01-01' 
    AND DATE(birthdate) <= '1997-01-01' THEN '1997-01-01' WHEN DATE(t1.enrollment_date) = '1900-01-01' 
    AND DATE(birthdate) > '1997-01-01' THEN DATE_ADD(birthdate, INTERVAL 30 DAY) WHEN DATE(t1.enrollment_date) > '1900-01-01' 
    AND DATE(birthdate) > DATE(t1.enrollment_date) THEN DATE_ADD(birthdate, INTERVAL 30 DAY) WHEN DATE(t1.enrollment_date) > '1900-01-01' 
    AND DATE(birthdate) <= DATE(t1.enrollment_date) THEN t1.enrollment_date END AS 'RegistrationAtCCC', 
    ...
    AND DATE(birthdate) > '1997-01-01' THEN DATE_ADD(birthdate, INTERVAL 30 DAY) WHEN DATE(t1.enrollment_date) > '1900-01-01' 
    AND DATE(birthdate) > DATE(t1.enrollment_date) THEN DATE_ADD(birthdate, INTERVAL 30 DAY) WHEN DATE(t1.enrollment_date) > '1900-01-01' 
    AND DATE(birthdate) <= DATE(t1.enrollment_date) THEN t1.enrollment_date END AS 'DateConfirmedHIVPositive', 
    NULL AS 'PreviousARTExposure', 
    NULL AS 'PreviousARTStartDate', 
    case when date_format(@endDate, "%Y-%m-01") > t1.death_date then @status := "dead" when date_format(@endDate, "%Y-%m-01") > date_format(transfer_out_date, "%Y-%m-01") then @status := "transfer_out" when timestampdiff(
      day, 
      if(
        rtc_date, 
        rtc_date, 
        date_add(
          encounter_datetime, interval 28 day
        )
      ), 
      @endDate
    ) <= 28 then @status := "active" when timestampdiff(
      day, 
      if(
        rtc_date, 
        rtc_date, 
    ...
    and 90 then @status := "defaulter" when timestampdiff(
      day, 
      if(
        rtc_date, 
        rtc_date, 
      ...
      @endDate
    ) > 90 then @status := "ltfu" else @status := "unknown" end as 'StatusAtCCC', 
    ...
    t1.encounter_datetime AS RecordCreatedOn, 
    t1.date_created AS RecordModifiedOn, 
    pm.Occupation AS 'Occupation', 
    NULL AS 'DateCreated' 
  FROM 
    etl.flat_hiv_summary_v15b t1 
    INNER JOIN ndwr_all_patients_build_queue__0 t3 ON (t3.person_id = t1.person_id) 
    left join ndwr_patient_pkv_occcupation_mapping pm on (t1.person_id = pm.person_id) 
    JOIN ndwr.mfl_codes mfl ON (mfl.location_id = t1.location_id) 
    left join ndwr.mfl_codes mfl2 on (
      mfl2.location_id = t1.last_non_transit_location_id
    ) 
    JOIN amrs.location l ON (l.location_id = t1.location_id) 
    JOIN amrs.location l2 ON (
      l2.location_id = t1.last_non_transit_location_id
    ) 
  WHERE 
    t1.is_clinical_encounter = 1 
    AND t1.next_clinical_datetime_hiv IS NULL 
  ORDER BY 
    t1.encounter_datetime DESC
);
SELECT 
...
SET 
  @dyn_sql = CONCAT(
    'replace into ', @write_table, '(select * from ndwr_all_patients_interim)'
  );
PREPARE s1 
from 
  @dyn_sql;
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' t1 join ndwr_all_patients_build_queue__0 t2 using (person_id);'
  );
PREPARE s1 
from 
  @dyn_sql;
