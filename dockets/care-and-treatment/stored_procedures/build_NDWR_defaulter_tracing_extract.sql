DELIMITER $$
CREATE PROCEDURE `build_NDWR_defaulter_tracing_extract`(
...
set 
  @primary_table := "ndwr_defaulter_tracing_extract";
set 
  @total_rows_written = 0;
set 
  @start = now();
set 
  @table_version = "ndwr_defaulter_tracing_extract_v1.0";
set 
  @query_type = query_type;
CREATE TABLE IF NOT EXISTS ndwr_defaulter_tracing_extract (
...
set 
  @write_table = concat(
    "ndwr_defaulter_tracing_extract_temp_", 
    queue_number
  );
set 
  @queue_table = concat(
    "ndwr_defaulter_tracing_extract_build_queue_", 
    queue_number
  );
SET 
  @dyn_sql = CONCAT(
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' (select * from ndwr_defaulter_tracing_extract_build_queue limit ', 
    queue_size, ');'
  );
PREPARE s1 
from 
...
SET 
  @dyn_sql = CONCAT(
    'delete t1 from ndwr_defaulter_tracing_extract_build_queue t1 join ', 
...
DEALLOCATE PREPARE s1;
set 
  @queue_table = "ndwr_defaulter_tracing_extract_sync_queue";
...
set 
  @loop_start_time = now();
drop 
  temporary table if exists ndwr_defaulter_tracing_extract_build_queue__0;
SET 
  @dyn_sql = CONCAT(
    'create temporary table if not exists ndwr_defaulter_tracing_extract_build_queue__0 (person_id int primary key) (select * from ', 
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' t1 join ndwr_defaulter_tracing_extract_build_queue__0 t2 on (t1.PatientPK = t2.person_id);'
  );
PREPARE s1 
from 
  @dyn_sql;
...
SELECT 
  CONCAT(
    'Creating missed appointments reasons .... '
  );
drop 
  temporary table if exists ndwr_missed_appointment_reasons;
...
drop 
  temporary table if exists ndwr_defaulter_tracing_extract_interim;
SELECT 
  CONCAT(
...
create temporary table ndwr_defaulter_tracing_extract_interim (
  SELECT 
    ...
    CASE WHEN e.obs REGEXP '!!1558=1555' THEN 'Phone follow up' WHEN e.obs REGEXP '!!1558=7066' THEN 'Home visit' ELSE NULL END AS 'TracingType', 
    CASE WHEN e.obs REGEXP '!!9600=1065' THEN 'Contact' WHEN e.obs REGEXP '!!9600=1066' THEN 'No Contact' WHEN e.obs REGEXP '!!1559=1065' THEN 'Contact' WHEN e.obs REGEXP '!!1559=1066' THEN 'No Contact' ELSE NULL END AS 'TracingOutcome', 
    1 as AttemptNumber, 
    null as IsFinalTrace, 
    null as TrueStatus, 
    mr.reason_for_missed_appointment as ReasonForMissedAppointment, 
    CASE WHEN e.obs REGEXP '!!1573=58' THEN 'TUBERCULOSIS' WHEN e.obs REGEXP '!!1573=6483' THEN 'CANCER' WHEN e.obs REGEXP '!!1573=123' THEN 'Infectious and parasitic disease (malaria, typhoid, cholera)' WHEN e.obs REGEXP '!!1573=10365' THEN 'NON INFECTIOUS HIV RELATED DISEASE' WHEN e.obs REGEXP '!!1573=903' THEN 'Other natural causes (heart disease, diabetes, hypertension)' WHEN e.obs REGEXP '!!1573=1572' THEN 'Non-natural causes (accident, murder, suicide, war, trauma)' WHEN e.obs REGEXP '!!1573=6483' THEN 'UNKNOWN' ELSE NULL END AS 'CauseOfDeath', 
    CASE WHEN e.obs REGEXP '!!9467=' THEN etl.GetValues(e.obs, 9467) ELSE NULL END AS 'Comments', 
    CASE WHEN e.obs REGEXP '!!5096=' THEN etl.GetValues(e.obs, 5096) ELSE NULL END AS 'BookingDate', 
    a.RecordCreatedOn, 
    a.RecordModifiedOn, 
    null as 'DateCreated' 
  FROM 
...
SET 
  @dyn_sql = CONCAT(
    'replace into ', @write_table, '(select * from ndwr_defaulter_tracing_extract_interim)'
  );
PREPARE s1 
from 
  @dyn_sql;
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' t1 join ndwr_defaulter_tracing_extract_build_queue__0 t2 using (person_id);'
  );
PREPARE s1 
from 
  @dyn_sql;
