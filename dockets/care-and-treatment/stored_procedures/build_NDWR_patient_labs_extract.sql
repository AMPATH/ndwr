DELIMITER $$
CREATE PROCEDURE `build_NDWR_ndwr_patient_labs_extract`(
...
set 
  @primary_table := "ndwr_patient_labs_extract";
set 
  @total_rows_written = 0;
set 
  @start = now();
set 
  @table_version = "ndwr_patient_labs_extract_v1.1";
set 
...
set 
  @write_table = concat(
    "ndwr_patient_labs_extract_temp_", 
    queue_number
  );
set 
  @queue_table = concat(
    "ndwr_patient_labs_extract_build_queue_", 
    queue_number
  );
SET 
  @dyn_sql = CONCAT(
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' (select * from ndwr_patient_labs_extract_build_queue limit ', 
    queue_size, ');'
  );
PREPARE s1 
from 
...
SET 
  @dyn_sql = CONCAT(
    'delete t1 from ndwr_patient_labs_extract_build_queue t1 join ', 
...
DEALLOCATE PREPARE s1;
set 
  @queue_table = "ndwr_patient_labs_extract_sync_queue";
...
set 
  @loop_start_time = now();
drop 
  table if exists ndwr_patient_labs_extract_build_queue__0;
SET 
  @dyn_sql = CONCAT(
    'create table if not exists ndwr_patient_labs_extract_build_queue__0 (person_id int primary key) (select * from ', 
...
DEALLOCATE PREPARE s1;
drop 
  temporary table if exists ndwr_patient_labs_extract_interim;
CREATE temporary TABLE ndwr.ndwr_patient_labs_extract_interim(
  SELECT 
    ...
    IF(
      f.encounter_id IS NOT NULL, f.encounter_id, 
      f.obs_id
    ) as 'VisitID', 
    IF(
      o.date_activated IS NOT NULL, o.date_activated, 
      f.obs_datetime
    ) AS 'OrderedbyDate', 
    f.obs_datetime as 'ReportedbyDate', 
    CASE WHEN f.concept_id = 856 THEN 'VL' WHEN f.concept_id = 730 THEN 'CD4 %' WHEN f.concept_id = 5497 THEN 'CD4 Count' ELSE NULL END AS 'TestName', 
  ...
  FROM 
    ...
    LEFT OUTER JOIN amrs.obs `t7` ON (
      o.order_id = t7.order_id 
...
SET 
  @dyn_sql = CONCAT(
    'replace into ', @write_table, '(select * from ndwr_patient_labs_extract_interim i)'
  );
PREPARE s1 
from 
  @dyn_sql;
...
SET 
  @dyn_sql = CONCAT(
    ...
    ' t1 join ndwr_patient_labs_extract_build_queue__0 t2 using (person_id);'
  );
PREPARE s1 
from 
  @dyn_sql;
